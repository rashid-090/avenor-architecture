...
function PortfolioListContent({ initialProjects = [] }) {
  ...
  const [projectsList, setProjectsList] = useState(
    initialProjects.length > 0 ? initialProjects : portfolioProjects
  );
  ...
  // Helper render function for grid
  const renderCard = (item, idx) => {
    const isAboveFold = currentPage === 1 && idx < 4; // Top 4 items visible above the fold on first page

    return (
      <Link
        key={item.id}
        href={`/projects/${item.slug}`}
        className="portfolio-card flex flex-col group cursor-pointer w-full"
      >
  ...
  return (
    <section ref={containerRef} className="bg-white text-zinc-950 pb-16 md:py-24">
      <div className="w-[95%] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 gap-y-10 items-start">
          {isPending
            ...
            : currentProjects.map((item, idx) => renderCard(item, idx))
          }
        </div>
...
export default function PortfolioList({ initialProjects = [] }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <PortfolioListContent initialProjects={initialProjects} />
    </Suspense>
  );
}
